/* Copy of glibc search.c, version 2.29, for use with tsearch(), tfind(), tdelete(), tdestroy()
...
// tq84 #include <assert.h>
// tq84 #include <stdalign.h>
#include <stddef.h>
#include <stdlib.h>
#include <string.h>
...
void *
tq84_tsearch (const void *key, void **vrootp, tq84__compar_fn_t compar)
{
  node q, root;
...
/* Find datum in search tree.
   ...
   COMPAR the ordering function.  */
void *
tq84_tfind (const void *key, void *const *vrootp, tq84__compar_fn_t compar)
{
  node root;
  node *rootp = (node *) vrootp;
...
void *
tq84_tdelete (const void *key, void **vrootp, tq84__compar_fn_t compar)
{
  node p, q, r, retval;
  int cmp;
  node *rootp = (node *) vrootp;
  ...
  int stacksize = 40;
  int sp = 0;
  node **nodestack = malloc(sizeof (node *) * stacksize);
  ...
  while ((cmp = (*compar) (key, root->key)) != 0)
      ...
      if (sp == stacksize)
	{
	  node **newstack;
	  stacksize += 20;
	  newstack = malloc(sizeof (node *) * stacksize);
...
static void
trecurse (const void *vroot, tq84__action_fn_t action, int level)
{
...
/* Walk the nodes of a tree.
   ...
   called at each node.  */
void
tq84_twalk (const void *vroot, tq84__action_fn_t action)
{
  const_node root = (const_node) vroot;
...
static void
tdestroy_recurse (node root, tq84__free_fn_t freefct)
{
  if (LEFT(root) != NULL)
